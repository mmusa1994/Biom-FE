...
const BiomTypes: React.FC<TBiomTypes> = ({ message, types }) => {
  return (
    <div className="flex flex-col md:flex-row items-center justify-end w-full max-w-[1440px] gap-8 md:gap-0 mt-10">
      ...
      <div className="flex items-center gap-0">
        {types?.length &&
          types?.map((el: TTypes, i: number) => (
            <figure key={el.id}>
              <ProgressiveImage
                className="min-w-[130px] h-[130px] md:min-w-[360px] md:h-[360px]"
                src={el.image}
