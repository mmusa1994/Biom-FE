  ...
  return (
    <div className="flex flex-col md:flex-row items-center md:items-start justify-center xl:justify-end w-full max-w-[1440px] gap-8 md:gap-0 mt-10 overflow-hidden">
      <div className="max-w-[297px] flex flex-col gap-8 xl:mt-10">
        <h4 className="chronicle-display font-extrabold text-4xl leading-[34px] xl:leading-[44px] text-black">
          {message}
        </h4>
        <Button
      ...
      <div className="flex items-center gap-0">
        {types?.map((el: TTypes, i: number) => (
          <div key={el.id} className="relative flex items-end group">
            <div className="absolute left-0 bottom-0 md:bottom-10 flex-col items-start justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-300 xl:min-w-[163px] xl:h-[52px] ">
              <h3 className="chronicle-display font-bold text-black text-[11px] md:text-[22px] leading-[28px]">
                {el.textHead}
              </h3>
              <label className="brandon-grotesque uppercase text-black text-[9px] md:text-[14px] leading-[28px] tracking-widest">
                {el.model}
              </label>
            </div>

            <figure className="transition-transform duration-300 ease-in-out group-hover:md:translate-x-[100px] group-hover:translate-x-[55px] ">
              <ProgressiveImage
